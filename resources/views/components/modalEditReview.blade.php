...
<div id="edit-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
            ...
            <!-- Modal body -->
            <form action="{{ route('books.reviews.edit', $book->id) }}" method="post" class="p-4 md:p-5">
                @csrf
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2">
                        ...
                        <div role="radiogroup" aria-label="Calificación" class="rating flex flex-row-reverse items-center gap-1 mb-3">
                            ...
                            @php $currentRating = old('rating', $review->rating ?? 5); @endphp
                            @for ($i = 5; $i >= 1; $i--)
                                @php $inputId = 'rating_edit_'.$review->id.'_'. $i; @endphp
                                <input type="radio" name="rating" id="{{ $inputId }}" value="{{ $i }}" class="sr-only peer" {{ $i == $currentRating ? 'checked' : '' }} aria-label="{{ $i }} estrellas">
                                <label for="{{ $inputId }}" title="{{ $i }} estrellas" class="cursor-pointer text-gray-300 hover:text-yellow-400 transition peer-checked:text-yellow-400 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-yellow-300 rounded">
                ...
                <div class="mt-4 flex gap-2">
                    ...
                    <!-- Botón Eliminar -->
                    <form action="{{ route('books.reviews.destroy', $book->id) }}" method="post" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-white inline-flex items-center bg-red-700 hover:bg-red-800 
                            focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 
                            text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">
                            <svg class="w-4 h-4 me-1" fill="#8e8e8e" viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
